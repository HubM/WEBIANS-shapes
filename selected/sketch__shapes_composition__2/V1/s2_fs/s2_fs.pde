...
void setup() {
 size(1748, 2480, PDF, "exports/shapes-2-FS-theme3-half-OP.pdf");
 ...
 for(int i = 50; i < width - 50; i+= 40) {
  for (int y = 50; y < height - 50; y+= 40) {
    ...
    int choosenColor = colorTheme3[int(random(1,4))];
    
    if (isFilled) {
      fill(choosenColor, halfRandomOp);
    } else {
      noFill();
    }
   ...
   //stroke(colorTheme3[choosenColor], halfRandomOp);      
    ...
    stroke(choosenColor, halfRandomOp);
